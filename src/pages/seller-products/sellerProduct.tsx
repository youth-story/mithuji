...
const SellerProduct = () => {
  ...
  const [category, setCategory] = useState<AllCategoryData[]>([]);
  const [data, setData] = useState<Data[]>([]);
  const [searchCategoryData, setSearchCategoryData] = useState<CategoryData[]>([]);
const [searchProductData, setSearchProductData] = useState<SearchProductData[]>([]);
  ...
  const removeProduct = async() => {
    try {
      console.log("response sending...");
      // Use fetch to make the post request with the url and the data
      const response = await fetch(`${BASEURL}/product/remove/${productId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
  ...
  // const listItems = (category, product) => {
  //   return (
  ...
  //         {category.map((c, i) => {
  //           console.log(c);
  //           const product = products.find((p) => p.category_name === c.category_name);
  //           return (
  //             <tr key={i}>
  //               <td className="image-box">
  //                 <img className="product-img" src={`https://16.170.207.123.nip.io/api/v1/content/get/${product.images}`} alt={"ERROR.jpg"} />
  //               </td>
  //               <td>{product.name}</td>
  //               <td>{c.category_name}</td>
  //               <td className="time"> {product.price_range}
  //                 <MoreVertIcon className="icon" onClick={() => handleClick(product.id)} />
...
// console.log(d.id);
    ...
    return (
      <tr key={i}>
        <td className="image-box">
          <img className="product-img" src={`${BASEURL}/content/get/${d.images}`} alt={"ERROR.jpg"} />
        </td>
        <td>{d.name}</td>
<td>{category[i].name}</td>
        <td className="time"> {d.price_range}
          <MoreVertIcon className="icon" onClick={() => handleClick(d.id)} />
          {productId === d.id && (
            <ul className="dropdown-content">

              <li>
                <button className={"date"} onClick={handleChangeDate}><EditIcon className="icon1" />Edit Details</button>
                <button className={"remove"} onClick={() => handleRemoveProduct()}><DeleteIcon className="icon1" />Remove Product</button>
              </li>
            </ul>
          )}
  ...
  const getProducts = async() => {
    try {
      ...
      if (response.status === 200) {
        ...
        setProduct(data.response.product.content)
